    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            def hydra_main(
                ...
                # Load env_cfg and agent_cfg after applying CLI overrides for env_setup.task_name
                task_name = hydra_env_cfg["env_setup"]["task_name"]
                env_cfg = load_cfg_from_registry(task_name, "env_cfg_entry_point")
                agent_cfg = load_cfg_from_registry(
                    task_name, run_cfg.agent_setup.entry_point
                )

                # update the environment config from the CLI arguments
                ...
                # update the run config from CLI arguments
                run_cfg = rl_run_cfg_from_dict(
                    run_cfg,
                    run_config_name,
                    hydra_env_cfg,
                    env_cfg_class=env_cfg.__class__,
