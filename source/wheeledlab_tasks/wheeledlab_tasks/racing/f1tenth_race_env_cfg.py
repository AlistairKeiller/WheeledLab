...
def cross_track_dist(
    ...
    sq_ctd = torch.where(
        ...
        torch.where(
            poses[..., 1] > 0,  # Corners
            (
                torch.sqrt((poses[..., 1] - straight) ** 2 + poses[..., 0] ** 2)
                - track_radius
            )
            ** 2,  # Positive y Turn
            (
...
def energy_through_turn(env, straight: float):
    ...
    energy_through_turn = torch.where(
        torch.abs(poses[..., 1]) > straight, speed**2, 0.0
    )
...
class RaceRewardsCfg:
    ...
    side_slip = RewTerm(
        func=side_slip,
        weight=10.0,  # Increases over time
        params={
    ...
    turn_energy = RewTerm(
        func=energy_through_turn, weight=20.0, params={"straight": STRAIGHT}
    )
...
def cart_off_track(
    env, straight: float, corner_in_radius: float, corner_out_radius: float
):
...
@configclass
class F1tenthRaceRLEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        # Scene settings
        self.scene = F1tenthRaceSceneCfg(
